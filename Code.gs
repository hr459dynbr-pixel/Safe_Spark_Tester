const CONFIG = {
  ...
  // Use a direct MP4 URL that's CORS-enabled. Replace with your own video.
  // This is a test video - replace with your actual date video
  DATE_VIDEO_URL: 'https://www.w3schools.com/html/mov_bbb.mp4'
};
...
.video-container{position:relative;background:#000;border-radius:16px;overflow:hidden;margin:20px 0;width:100%;max-width:100%}
.video-wrapper{position:relative;width:100%;padding-bottom:56.25%;height:0}
.date-video{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover}
...
@media(max-width:768px){.user-video{width:120px;height:90px;bottom:10px;right:10px}.timer{font-size:48px}.choices{grid-template-columns:1fr}.activity-display h3{font-size:20px}}
</style></head><body><div class="wrap"><div id="app"></div></div><script>
const CONFIG={DATE_DURATION_SECONDS:180,VIDEO_URL:'https://www.w3schools.com/html/mov_bbb.mp4'};
